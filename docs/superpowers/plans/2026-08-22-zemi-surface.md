...
**Why:** `near = 0.5`, `minDistance = 15` and `maxDistance = 480` are galaxy-scale constants. A surface orbit radius is about 2, and the ground at the bottom of frame is `0.202 × shardRadius` away. At the recommended shard radius of 3.0 that is 0.61 — clearing `near = 0.5` by 0.11. Any smaller shard renders with a hole where the visitor's feet are. Dropping `near` globally to 0.05 is not the fix: `far` is 2000, and the spike measured visible z-fighting on distant bodies at that ratio.

**Measurement, resolved during this task:** planet framing (ratio ~5,600) renders clean — verified in the browser, no banding on arm dust or moons. `near` uses a 0.06 coefficient, which clears the ground constraint 3.4x over while holding the surface-scale ratio near 11,000, well below the 40,000 where banding was observed. Surface framing itself is unverifiable until Task 6 builds a surface; **Task 6 must re-check it.**
